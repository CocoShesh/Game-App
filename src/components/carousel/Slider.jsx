...
export default function Slider() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("/public/api/gamesData.json");
        if (!response.ok) {
        ...
        setData(jsonData);
        // Initialize isOpen array with false values for each slide
        setIsOpen(new Array(jsonData.length).fill(false));
      } catch (error) {
        console.error("Fetch error:", error);
      }
    };
    ...
    <>
      ...
      >
        ...
        <Swiper
          ...
          grabCursor={true}
          centeredSlides={true}
          slidesPerView={2} // Removed quotes and set slidesPerView as a number
          coverflowEffect={{
            rotate: 35,
            stretch: 250,
            depth: 200,
